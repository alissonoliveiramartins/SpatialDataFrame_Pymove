...
from pymove.core import MoveDataFrameAbstractModel, SpatialMoveDataFrameAbstractModel
...
from pymove.utils.trajectories import format_labels, format_labelsSpatial, shift
...
class SpatialDataFrame:
    ...
    def __new__(
        ...
        if type_ == "pandas":
            return PandasSpatialMoveDataFrame(
                data, latitude, longitude
            )
        
        if type_ == "dask":
            return DaskSpatialMoveDataFrame(
                data, latitude, longitude
            )
...
class PandasSpatialMoveDataFrame(pd.DataFrame, SpatialMoveDataFrameAbstractModel):
    def __init__(
        self,
        data,
        latitude=LATITUDE,
        ...
        Raises
        ------
            AttributeError if the data doesn't contains one of the columns LATITUDE, LONGITUDE
        """
        if isinstance(data, dict):
            data = pd.DataFrame.from_dict(data)
        elif (isinstance(data, list) or isinstance(data, np.ndarray)) and len(data) >= 4:
            zip_list = [LATITUDE, LONGITUDE]
            for i in range(len(data[0])):
                try:
                    zip_list[i] = zip_list[i]
                except KeyError:
        ...
        mapping_columns = format_labelsSpatial(latitude, longitude)
        tdf = data.rename(columns=mapping_columns)
    ...
    def _has_columns(data):
        """
        Checks whether the received dataset has 'lat', 'lon'
        columns.
    ...
    def len(self):
        """
        Returns the length/row numbers in dataframe.
    ...
    def get_users_number(self):
        ...
        """
        Check and return number of users in Points of Interest.
        ...
        -------
        number_ : int
            Represents the number of users in Points of Interest.
        """
        operation = begin_operation("get_users_numbers")
    ...
    def to_numpy(self):
        ...
        """
        Converts Points of Interest to numpy array format.
        ...
        -------
        numpy_ : np.array
            Represents the Points of interest in numpy array format.
        """
        operation = begin_operation("to_numpy")
        numpy_ = self._data.values
        self.last_operation = end_operation(operation)
    ...
    def to_dict(self):
        """
        Converts Points of Interest to dict format.
        ...
        -------
        dict_ : dict
            Represents the Points of Interest in dict format.
        """
        operation = begin_operation("to_dict")
        dict_ = self._data.to_dict()
        self.last_operation = end_operation(operation)
    ...
    #Esperando resposta de Gustavo Para Apagar...
    def to_grid(self, cell_size, meters_by_degree=lat_meters(-3.8162973555)):
        """
        Converts Points of interest to grid format.
        ...
        -------
        grid_ : pymove.core.grid
            Represents the Points of interest in grid format.
        """
        operation = begin_operation("to_grid")
        grid_ = Grid(self, cell_size, meters_by_degree)
        self.last_operation = end_operation(operation)
    ...
    def to_DataFrame(self):
        """
        Converts Points of Interest to DataFrame format.
        ...
        -------
        pandas.core.DataFrame
            Represents the Points of interest in DataFrame format.
        """
        operation = begin_operation("to_DataFrame")
        data_ = self._data
        self.last_operation = end_operation(operation)
        ...
        if inplace:
            ...
            if labels is not None:
                ...
                if isinstance(columns, str):
                    ...
                    _labels2 = set(columns)
            _columns = set(["lat", "lon"])
            if (axis == 1 or axis == "columns" or columns) and (
                _labels1.union(_labels2) & _columns
            ):
                raise AttributeError(
                    "Could not drop columns lat and lon."
                )
...
class DaskSpatialMoveDataFrame(DataFrame, SpatialMoveDataFrameAbstractModel):
    def __init__(self,
        data,
        latitude=LATITUDE,
        longitude=LONGITUDE,
    ...
    def _has_columns(data):
        ...
        bool
            Represents whether or not you have the required columns.
        """
        if LATITUDE in data and LONGITUDE:
            return True
        return False
